...
def gen_user(choice):
    ...
    if choice == "8":
        c = d = random.choices(a)
        d = random.choices(e)
        f = [c[0], d[0], d[0], c[0], c[0], c[0], c[0]]
        random.shuffle(f)
        username = ''.join(f)
        if username in banned[0]:
            c = d = random.choices(a)
            d = random.choices(e)
            f = [c[0], d[0], d[0], d[0], d[0], d[0], c[0]]
            random.shuffle(f)
            username = ''.join(f)
        else:
            pass
        ...
        for i in range(int(msg[0])):
            ...
            if "Available" in isav:
                ...
                try:
                    ...
                    await event.client.send_file(event.chat_id, "https://t.me/x_o_x/698", caption=f'''
⌯ Source Max ! 🐊
⤷ User : @{username} 
⤷ Clicks : {trys} 
⤷ Type : #{choice}
...
''')
        ...
        isclaim.append("off")
        trys = ""
        await event.client.send_message(event.chat_id, "Done hunt #1 🔔")
